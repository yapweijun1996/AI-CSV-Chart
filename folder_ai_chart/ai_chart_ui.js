...
// NOTE: canonicalJobKey and deduplicateJobs functions moved to ai_chart_aggregates.js
/* 
...
// NOTE: addMissingDataWarning function moved to ai_chart_aggregates.js
/* 
...
/* ========= aggregates ========= */
// NOTE: Aggregation functions moved to ai_chart_aggregates.js
/* 
...
// NOTE: Chart functions moved to ai_chart_aggregates.js
/* 
...
// NOTE: renderChartCard and renderAggTable functions moved to ai_chart_aggregates.js
/* 
function renderChartCard(agg, chartsContainer, defaultType='auto', defaultTopN=20, titleForFile='chart', options = {}){
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 12px; align-items: end;">
      ...
      <label>Function:
        <select id="edit-function" style="width: 100%; margin-top: 4px;">
          <option value="sum" ${aggFunction === 'sum' ? 'selected' : ''}>Sum (Default for ERP)</option>
  ...
  editPanel.querySelector('#edit-apply').onclick = async () => {
    ...
    const parentCard = chartsContainer?.parentElement?.parentElement; // Go up to the main card (not cardContent)

    // Read current filter + missing settings from the card
    ...
    // Recompute aggregate for this card using included rows and current filter/missing settings
    ...
    // Update the current card's aggregate data (shared by all charts in this card)
    ...
    if (parentCard && parentCard.classList.contains('card')) {
      // Persist job definition so snapshots capture this edit
      console.log('🔧 Updating main card dataset with manual edits:', { newGroupBy, newMetric, finalFunction, newBucket });
      ...
      // Update canonical key so downstream features remain consistent
      if (typeof canonicalJobKey === 'function') {
        parentCard.dataset.canonicalKey = canonicalJobKey({
          groupBy: newGroupBy || '',
          metric: newMetric || '',
          agg: finalFunction || 'sum',
          dateBucket: newBucket || ''
        });
      }

      // Update the top-of-card title (avoid the AI explanation title)
      const headTitle = parentCard.querySelector(':scope > .card-head .card-title')
                     || parentCard.querySelector('.card-head .card-title');
      if (headTitle) {
        headTitle.textContent = `${finalFunction}(${newMetric || ''}) by ${newGroupBy}`;
      }

      // Update card subheader
      const cardSub = parentCard.querySelector(':scope > .card-head .card-sub')
                   || parentCard.querySelector('.card-head .card-sub');
      if (cardSub) {
        cardSub.textContent = `${newAgg.rows.length} groups · ${newAgg.header[1]}`;
      }

      // Clear and re-add missing data warning
      parentCard.querySelectorAll('.missing-data-warning').forEach(w => w.remove());
      addMissingDataWarning(parentCard, newAgg, getIncludedRows().length, !!showMissingFlag);

      // Re-render table with correct showMissing
      const tableBox = parentCard.querySelector('.table-wrap');
      if (tableBox) {
        renderAggTable(newAgg, tableBox, 20, !!showMissingFlag, { formatNumberFull });
      }

      // Redraw ALL chart cards in this parent (not only the one being edited)
      if (chartsContainer) {
        chartsContainer.querySelectorAll('.chart-card').forEach(cc => {
      ...
      // Keep layout stable after edits
      requestAnimationFrame(() => {
        try { applyMasonryLayout(); } catch {}
      });
    } else {
      console.error('❌ Could not find parent card for dataset update. DOM structure:', {
        chartCard, chartsContainer, parentCard,
        parentCardClasses: parentCard?.className
      });
    }

    // Regenerate AI explanation for this card with the updated config (if API key available)
    if (parentCard) {
      try {
        await generateExplanation(newAgg, {
          groupBy: newGroupBy || '',
          metric: newMetric,
          agg: finalFunction || 'sum',
          dateBucket: newBucket || ''
        ...
        console.warn('Failed to regenerate explanation after edit apply:', e);
      }
    }
...
function renderAggTable(agg, container, previewN=10, showMissing = false){
  ...
  function renderTableFooter() {
    ...
    // First cell labels
    const labelCell1 = document.createElement('td'); labelCell1.textContent = 'Σ visible (table filter)'; labelCell1.className = 'footer-label';
    const labelCell2 = document.createElement('td'); labelCell2.textContent = 'Σ table (post-minGroupShare)'; labelCell2.className = 'footer-label';
    const labelCell3 = document.createElement('td'); labelCell3.textContent = 'Σ total (pre-filter)'; labelCell3.className = 'footer-label';
    ...
    const labelCell6 = document.createElement('td'); labelCell6.textContent = 'Σ Total Record'; labelCell6.className = 'footer-label';
...
async function workerAggregateWithFallback(rows, profile, plan, timeoutMs = 15000) {
  ...
  return new Promise((resolve) => {
    const timer = setTimeout(() => {
      ...
      showToast('Worker timed out — falling back to main thread.', 'info');
    ...
    w.onmessage = (e) => {
      ...
      if (e.data && !e.data.error) {
        ...
        showToast('Worker failed — falling back to main thread.', 'info');
        resolve(localAggregate());
      }
      try { w.terminate(); } catch {}
    };
